...
parser.add_argument('--exp_name', default='pretrain2018', type=str, help='exp name')
...
def main(args):
    ...
    criterion_memory = memory_Loss(CC_modalities, CC_classes, 0.5, 1.0).cuda()  # modal_num = 4

    optimizer = torch.optim.Adam([{"params": ori_para}, {"params": limage, "lr": 0.005}], lr=args.lr,
    ...
    for epoch in range(args.epochs):
        ...
        try:
            ...
            total_losses_ = AverageMeter('Loss', ':.4e')

            mode = "train" if model_1.training else "val"
            batch_per_epoch = len(train_loader)
            progress = ProgressMeter(
                batch_per_epoch,
                [batch_time, data_time, losses_, total_losses_],
            ...
            for i, batch in enumerate(zip(train_loader)):
                ...
                optimizer.zero_grad()
                out, mask_ratio, _, _ = model_1(inputs_S1, patch_locations)

                if args.memoryContrast:  # class contrast
                    kk = 0
                    modulContrast_loss = 0
                    for m1 in range(CC_modalities):
                        for m2 in range(m1 + 1, CC_modalities):
                            modulContrast_loss = modulContrast_loss + criterion_memory.forward_modal_cos(
                                model_1.unet._cluster_centers.weight.view(-1, CC_modalities, CC_classes)[:, m1, :],
                                model_1.unet._cluster_centers.weight.view(-1, CC_modalities, CC_classes)[:, m2, :])
                            kk = kk + 1
                    modulContrast_loss = modulContrast_loss / kk
                loss_ = torch.pow((inputs_S1 - out['uout_reconstruction']), 2).mean()
                loss = loss_ + modulContrast_loss * 0.01
                ...
                if not np.isnan(loss_.item()):
                    ...
                    total_losses_.update(loss.item())
                ...
                t_writer_1.add_scalar(f"Total Loss/{mode}{''}",
                                      loss.item(),
                                      global_step=batch_per_epoch * epoch + i)
            ...
            t_writer_1.add_scalar(f"Summary Loss/Pretrain", losses_.avg, epoch)
            t_writer_1.add_scalar(f"Summary Total Loss/Pretrain", total_losses_.avg, epoch)
