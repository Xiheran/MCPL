...
parser.add_argument('--all_data', default=False, type=bool,
                    help='if train, when part != 1, do semi-supervise when finetune')
parser.add_argument('--memoryContrast', default=True, type=bool, help='if memoryContrast')
...
parser.add_argument('--weight_kl', default=0.1, type=float, help="train sub")
...
def main(args):
    ...
    criterion_memory = memory_Loss(4, 3, 0.5, 1.0).cuda()  # modal_num = 4, class_num = 3, t = 0.5,  no_use = 1
    ...
    optimizer = torch.optim.Adam(ori_para, lr=args.lr, weight_decay=args.weight_decay)  # weight_decay 0.0001

    if args.val != 0:
        full_train_dataset, l_val_dataset, _, _labeled_name_list = get_datasets_train_rf_withvalid(
            args.seed,
    ...
    for epoch in range(args.epochs):
        ...
        try:
            ...
            totalloss = AverageMeter('Loss', ':.4e')
            ...
            for i, batch in enumerate(zip(train_loader)):
                ...
                if not batch[0]["patient_id"][0] in _labeled_name_list and (
                        np.random.random() > args.semi_proportion):  # or epoch < 50):
                    continue
                ...
                for ci in range(args.batch_size):
                    mask = MaskEmbeeding1(1, raw_input=raw_input, mdp=args.mdp, mask=False,
                                          patch_shape=args.patch_shape)  # do not do patch mask but do modal mask

                    inputs_S1[ci] = inputs_S1[ci] * mask + limage[:, :, patch_locations[0][0]: patch_locations[0][1],
                                                           patch_locations[1][0]: patch_locations[1][1],
                                                           patch_locations[2][0]: patch_locations[2][1]] * (
                                            1 - mask)  # not
                    mask_code = mask.mean((2, 3, 4))
                    for l in range(mask_code.shape[0]):
                        mask_codes.append((mask_code[l, 0] * 8 + mask_code[l, 1] * 4 + mask_code[l, 2] * 2 + mask_code[
                            l, 3] * 1 - 1).astype(int))

                inputs_S1, labels_S1 = args.train_transforms(
                    [inputs_S1.transpose(0, 2, 3, 4, 1), labels_S1.transpose(0, 2, 3, 4, 1)])

                inputs_S1, labels_S1 = [torch.from_numpy(np.ascontiguousarray(x.transpose(0, 4, 1, 2, 3))).float() for x
                                        in [inputs_S1, labels_S1]]
                ...
                if args.memoryContrast:  # class contrast
                    kk = 0
                    classContrast_loss = 0
                    for c1 in range(CC_classes):
                        for c2 in range(c1 + 1, CC_classes):
                            classContrast_loss = classContrast_loss + criterion_memory.forward_class_cos(
                                model_1.module.unet._cluster_centers.weight.view(-1, CC_modalities, CC_classes)[:, :,
                                c1],
                                model_1.module.unet._cluster_centers.weight.view(-1, CC_modalities, CC_classes)[:, :,
                                c2])
                            kk = kk + 1
                    memoryContrast_loss = classContrast_loss / kk
                ...
                if not args.deep_supervised:
                    ...
                    for l in range(segs_S1[0].shape[0]):
                        ...
                        mean_results[mask_codes[l], :] = mean_results[mask_codes[l], :] * 0.99 + torch.tensor(
                            [l.detach().cpu() for l in metric_[0]]) * 0.01
                ...
                if args.batch_size == 2:
                    if args.use_weight and epoch > 50:
                        ...
                        for ii in range(labels_S1.shape[1]):
                            weight1.append(
                                CE_L(torch.sigmoid(segs_S1[-1][:1, ii, :, :, :].detach()), labels_S1[:1, ii, :, :, :]))
                            weight2.append(
                                CE_L(torch.sigmoid(segs_S1[-1][1:, ii, :, :, :].detach()), labels_S1[1:, ii, :, :, :]))
                        ...
                        loss_kl = F.mse_loss(style[args.feature_level + 1][0], style[args.feature_level + 1][1],
                                             reduction="none").mean()
                ...
                loss = loss + 0.01 * memoryContrast_loss
                loss.backward()
                optimizer.step()
            ...
            t_writer_1.add_scalar(f"TotalLoss/train", totalloss.avg, epoch)
            ...
            if args.val != 0:
                if (epoch + 1) % args.val == 0 and epoch > 301:
                    total = 0
...
def step(data_loader, model, args, criterion: EDiceLoss_Val, metric, epoch, writer, save_folder=None,
         patients_perf=None, mask_modal=[], patch_shape=128):
    # Setup
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
