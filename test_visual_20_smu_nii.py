...
from dataset.brats import get_datasets_train_rf_withvalid, get_datasets_train_rf_withtest_20nii
...
parser.add_argument('--exp_name', default='test20_visual', type=str, help='exp name')

parser.add_argument('--val', default=0, type=int, help="how often to perform validation step")  # test DataSet
# parser.add_argument('--val', default=20, type=int, help="how often to perform validation step")   # validation DataSet
...
def main(args):
    ...
    model_1 = Unet_missing(input_shape=[128, 128, 128], out_channels=3, mdp=3, init_channels=16, pre_train=False,
    ...
    if args.val != 0:  # default 20
        full_train_dataset, l_val_dataset, _, _ = get_datasets_train_rf_withvalid(args.seed, fold_number=args.fold)
    else:
        full_train_dataset, l_val_dataset, _, _ = get_datasets_train_rf_withtest_20nii(args.seed, fold_number=args.fold)
...
def step(data_loader, model, model_temp, criterion: EDiceLoss_Val, metric, epoch, writer, save_folder=None,
         patients_perf=None, args=None):
    # Setup
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    ...
    for i, val_data in enumerate(data_loader):
        ...
        hd95 = []
        dice = []
        for l in range(segs.shape[1]):  # shape[1] 类别?
    ...
    save_every_metrics(epoch, metrics, writer, epoch, False, save_folder)
    ...
    # file = open("runs_Kmax_2tasks/test20_full_visual/smu20.csv", "a+")
    # file = open("runs_Kmax_2tasks_beforeMul_cos/_ET62_1122_899/smu20.csv", "a+")
    file = open("Reslut_try/_250922/fineturn20.csv", "a+")
