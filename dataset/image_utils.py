...
def irm_min_max_preprocess(image, low_perc=1, high_perc=99):
    ...
    non_zeros = image > 0
    low, high = np.percentile(image[non_zeros], [low_perc, high_perc])  # 18float64
    image = np.clip(image, low, high)  # 18float64
    image = normalize(image)  # 18float64
    return image
...
def noZeroed(img: np.ndarray) -> np.ndarray:
    slices = (img != 0)
    return slices


def zscore_reverse_normalise(img: np.ndarray, imag_mean, imag_std) -> np.ndarray:
    img = (img * imag_std) + imag_mean
    return img


def zscore_mean_std(img: np.ndarray) -> np.ndarray:
    slices = (img != 0)
    return np.mean(img[slices]), np.std(img[slices])
