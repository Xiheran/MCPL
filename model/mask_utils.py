...
def MaskEmbeeding1(B, mask_ratio=0.75, raw_input=None, patch_size=16, token_index=[x for x in range(0, 1372)], mdp=0,
                   mask=True, mask_modal=[], patch_shape=128):
    ...
    decoder_embeeding = decoder_embeeding.reshape(B, 4, D // patch_size, H // patch_size, W // patch_size, patch_size,
                                                  patch_size, patch_size).transpose(0, 1, 2, 5, 3, 6, 4, 7)
...
def masking(B, mask_ratio=0.125, patch_size=16, raw_size=[64, 192, 192]):
    ...
    decoder_embeeding = decoder_embeeding.reshape(B, 1, raw_size[0] // patch_size, raw_size[1] // patch_size,
                                                  raw_size[2] // patch_size, patch_size, patch_size,
                                                  patch_size).transpose(0, 1, 2, 5, 3, 6, 4, 7)
